				...
				if (field.meta.exists( ':forward' )) {
					...
					for (cfield in cfields) {
						...
						switch (cfield.kind) {
							...
							case FMethod(m):
								nfield.toFFun().mkPublic().mkInline();
								
								var args = nfield.getMethod().args = cfield.type.args().toFunctionArgs();
								
								nfield.body( macro { 
									return $e { Context.parse( 
										'${field.name}.${cfield.name}(${args.map(function(a) return a.name).join(",")})', 
										field.pos ) 
									}; 
								} );
